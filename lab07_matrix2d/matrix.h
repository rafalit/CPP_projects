...
public:
    ...
    TwoDimensionMatrix(const TwoDimensionMatrix & matrix__);

    TwoDimensionMatrix(const MatrixElement matrix[size_][size_]);

    void set(size_t row, size_t column, MatrixElement value){
        matrix_[row][column] = value;
    }

    size_t get(size_t row, size_t column)const{
        return matrix_[row][column];
    }

    TwoDimensionMatrix result(const TwoDimensionMatrix& matrix1, const TwoDimensionMatrix& matrix2) const;

    TwoDimensionMatrix& operator = (const TwoDimensionMatrix& m);

    std::ostream& operator<<(std::ostream& ma) const;
    ...
    TwoDimensionMatrix& operator *= (double number);
